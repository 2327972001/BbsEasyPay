...
<template id="editOnlinePaymentInterface-template">
	<div>
		<div class="main" >
            ...
            <span v-if="interfaceProduct == 5">易支付</span>
					</el-form-item>
          ...
          <el-form-item label="易支付ID" v-if="interfaceProduct ==5" :required="true" :error="error.easy_app_id">
            <el-input type="textarea" v-model="easy_app_id" :autosize="{minRows: 1}" placeholder="请输入易支付ID" ></el-input>
          </el-form-item>
          <el-form-item label="易支付密钥" v-if="interfaceProduct ==5" :required="true" :error="error.easy_epay_public_key">
            <el-input type="textarea" v-model="easy_epay_public_key" :autosize="{minRows: 5}" placeholder="请输入易支付密钥" ></el-input>
          </el-form-item>
			    ...
			    if(result){
			    	...
			    	if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			...
			    			}else if(key == "epayEasy"){
                  let epayEasy = mapData[key];
                  _self.easy_app_id = epayEasy.app_id;
                  _self.easy_epay_public_key = epayEasy.epay_public_key;
                }
			    		}
