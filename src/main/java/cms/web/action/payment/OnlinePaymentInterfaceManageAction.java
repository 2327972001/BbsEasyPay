...
public class OnlinePaymentInterfaceManageAction{
	...
	public String add(ModelMap model,OnlinePaymentInterface formbean,BindingResult result,	
		...
		if(formbean.getInterfaceProduct() != null){
			...
			}else if(formbean.getInterfaceProduct().equals(5)){//5.易支付
				String easy_app_id = request.getParameter("easy_app_id");
				String easy_epay_public_key = request.getParameter("easy_epay_public_key");

				if(easy_app_id != null && !"".equals(easy_app_id.trim())){
					epay_easy.setApp_id(easy_app_id);
				}else{
					error.put("easy_app_id", "易支付ID不能为空");
				}

				if(easy_epay_public_key != null && !"".equals(easy_epay_public_key.trim())){
					epay_easy.setEpay_public_key(easy_epay_public_key);
				}else{
					error.put("easy_epay_public_key", "易支付密钥不能为空");
				}

				dynamicParameter = JsonUtils.toJSONString(epay_easy);
				onlinePaymentInterface.setSupportEquipment(onlinePaymentInterfaceManage.setSupportEquipment(true, true, false));
			}else{
				result.rejectValue("interfaceProduct","errors.required", new String[]{"请选择接口产品"},"");
			}
		}
