...
public class PaymentFormAction {
	...
	@Resource EpayConfig_PC epayConfig_PC;
				...
				if(error.size() == 0){
					if(rechargeAmount.compareTo(new BigDecimal("0")) >0){
						...
						//	 return "redirect:/api";   String.
							String createHtmlText = this.jumpPayAPI(isAjax,interfaceProduct,paymentModule,paymentRequestId,user.getId(),"USER_ID:"+user.getId().toString(),String.valueOf(rechargeAmount),code,request, response);
							if(isAjax == true){
								returnValue.put("success", "true");
	...
	public String notify(ModelMap model,@PathVariable Integer interfaceProduct,
		...
		if(interfaceProduct.equals(1)){//1.支付宝即时到账
			...
			//验证签名
			boolean signVerified = epayNotify.verifyNotify(request,response);
			if(signVerified){
				//商户订单号
				String out_trade_no = request.getParameter("out_trade_no");
				//支付宝交易号
	...
	private String jumpPayAPI(boolean isAjax,Integer interfaceProduct,Integer paymentModule,String paymentRunningNumber,Long parameterId,String orderName,String total,String code,
		...
		if(interfaceProduct.equals(1)){//1.支付宝即时到账 
			...
			*/
			...
			String return_url = Configuration.getUrl(request)+"paymentCompleted/"+interfaceProduct+"/"+paymentModule+"/"+parameterId;
			//易支付配置获取PID和KEY
			epayConfig_PC.GetPidOrKey(interfaceProduct);

			HashMap<String,String> sign = new HashMap<String,String>();
			sign.put("pid",epayConfig_PC.getPartner());
			sign.put("type","alipay");//支付方式
			sign.put("out_trade_no",paymentRunningNumber);
			sign.put("notify_url",notify_url);
			sign.put("return_url",return_url);
			sign.put("name",orderName);
			sign.put("money",total);

			// form表单生产
			EpaySubmit submit = new EpaySubmit(epayConfig_PC);
			String form = submit.buildRequestForm(sign);
			//建立请求
			createHtmlText = form;
			if(isAjax ==false){//如果为默认表单提交
				response.setContentType("text/html;charset="+epayConfig_PC.getInput_charset());
				response.setHeader("Cache-Control", "no-store"); //HTTP1.1
				response.setHeader("Pragma", "no-cache"); //HTTP1.0
				response.setDateHeader("Expires", 0);
				response.getWriter().print(form);
